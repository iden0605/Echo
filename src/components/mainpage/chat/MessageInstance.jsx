...
function MessageInstance({ userMessage, aiMessage, aiLoading, chatboxHeight, onEditMessage }) {
  ...
  useEffect(() => {
    if (chatboxHeight > 0) {
      ...
      const calculatedHeight = window.innerHeight - chatboxHeight - headerHeight - 32;
      ...
      >
        <div className="w-full pl-8 flex justify-end">
          <motion.div
            ...
            className={`relative px-4 py-3 rounded-3xl bg-stone-700 text-cream-50 text-left ${showReadMore ? 'pr-8 pt-6' : ''} ${isEditing ? 'ring-1 ring-stone-500 max-w-[95%] w-full' : 'max-w-[70%]'}`}
          >
            <motion.button
              onClick={handleEdit}
